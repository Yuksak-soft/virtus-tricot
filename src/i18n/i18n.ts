export const translationsUz = {
    bannerText: "Toshkentda sifatli to'qimachilik aksessuarlar ishlab chiqarish va sotish",
    formTitle: "Aloqa shakli",
    name: "Ism",
    phoneNumber: "Telefon raqami",
    send: "Jo'natish",
    statisticTextOne: "xorijiy malakali mutaxassislar",
    statisticTextTwo: "to'qimachilik aksessuarlarini ishlab chiqarishda ko'p yillik tajriba",
    statisticTextThree: "mijozlar eksport qiluvchi mijozlar",
    statisticTextFour: "mamnun mijozlar",
    statisticTextFive: "ishlab chiqarish uchun joy",
    statisticTextSix: "aksessuarlar turlari",
    statisticTextSeven: "har yili bajarilgan buyurtmalar",
    portfolio: "Portfoliolar",
    advantages: "Bizning afzalliklarimiz",
    advantage1: "Mijozga individual yondashuv",
    advantage2: "Yuqori darajadagi ishlab chiqarish",
    advantage3: "Qulay narxlar",
    advantage4: "Biz asosan orqaga qaytarish tizimida ishlamaymiz.",
    advantage5: "Malakali mutaxassislarning professional jamoasi",
    advantage6: "Zo'r sifat",
    advantage7: "Qulay va tez yetkazib berish",
    advantage8: "Biz belgilangan muddatlarga rioya qilishni kafolatlaymiz",
    clients: "Bizga ishonadigan mijozlarimiz",
    clientsText: "Biz uchun ish sifatining asosiy ko'rsatkichi mijozlarning ishonchidir. Biz mijozlarimizni qadrlaymiz va ular bizning kompaniyamizni uzoq muddatli hamkorlik uchun tanlaganidan faxrlanamiz.Hamkorlarimiz bilan ishonchli munosabatlar bizga eng murakkab muammolarni qisqa vaqt ichida hal qilish imkonini beradi. Avvalo, bizning mijozlarimiz bizning hamkorlarimiz va do'stlarimizdir, biz har bir hamkorimizni qadrlaymiz va har kuni xizmatimizni yaxshilashga harakat qilamiz!",
    footerText: "Barcha huquqlar himoylangan"

};

export const translationsRu = {
    bannerText: "Производство и продажа качественные текстильного аксессуары в Ташкенты",
    formTitle: "Форма контакт",
    name: "Имя",
    phoneNumber: "Телефон номер",
    send: "Отправить",
    statisticTextOne: "иностранные квалифицированнные специалистов",
    statisticTextTwo: "лет опыта изготовлении текстилного аксессуары",
    statisticTextThree: "клиенты экспортёр клиентов",
    statisticTextFour: "довольных клиентов",
    statisticTextFive: "место для производство",
    statisticTextSix: "видоа аксессуаров",
    statisticTextSeven: "выполненных заказов каждый год",
    portfolio: "Портфолио",
    advantages: "Наши преимущества",
    advantage1: "Индивидуальный подход к клиенту",
    advantage2: "Высокий уровень производство",
    advantage3: "Доступные цены",
    advantage4: "Мы приципиально не работаем по системы «откатов»",
    advantage5: "Профессиональная команда из квалифицированные специалистов",
    advantage6: "Отличное качества",
    advantage7: "Удобная и быстрая доставка",
    advantage8: "Гарантируем соблюдение сроков",
    clients: "Наши клиенты которые доверябт нам",
    clientsText: "Для нас главный показатель качества работы является доверие клиентов. Мы ценим своих клиентов и гордимся темб что они выбрали именно нашу компания для длительного сотрудничестваю Доверительные отношения с нашими партнёрами позволяют нам решать самые сложные задача в сжатые сроки. В первую очередь, наши клиенты - это наши пратнёры и друзья, мы ценим каждого нашего партнёра, и стараемся ежедневно улучшать наш сервис услуг!",
    footerText: "Все права защищены"
};